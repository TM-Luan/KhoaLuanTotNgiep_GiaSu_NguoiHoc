...
class TutorDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    print('>>> [TutorDetailPage] Received arguments raw type: ${args?.runtimeType}');

    Tutor? tutor;
    // ===> KIỂM TRA TYPE AN TOÀN HƠN <===
    if (args is Tutor) {
      tutor = args;
      print('>>> [TutorDetailPage] Successfully cast arguments to Tutor: ${tutor.name}');
    } else {
      print('>>> [TutorDetailPage] FAILED to cast arguments to Tutor or arguments are null/wrong type.');
    }
    ...
    print('>>> [TutorDetailPage] Tutor object received successfully. Building detail view for ${tutor.name}...');


    // --- Xác định avatarWidget ---
    final bool isPlaceholderImage = tutor.image.contains('pravatar.cc') || tutor.image.isEmpty; // Thêm kiểm tra rỗng
    Widget avatarWidget;
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Column(
                children: [
                  _buildInfoRow(context, Icons.phone_outlined, 'Số điện thoại', tutor.soDienThoai),
                  _buildInfoRow(context, Icons.email_outlined, 'Email', tutor.email),
                  _buildInfoRow(context, Icons.location_on_outlined, 'Địa chỉ', tutor.diaChi),
                  const Divider(height: 25, thickness: 1),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      _buildActionButton(
                          context, Icons.phone, 'Gọi điện',
                          (tutor.soDienThoai?.isNotEmpty ?? false)
                              ? () => _makePhoneCall(context, tutor?.soDienThoai)
                              : null
                      ),
                      _buildActionButton(
                          context, Icons.mail, 'Gửi Mail',
                ...
                child: Padding(
                  ...
                  child: Column(
                     ...
                     children: [
                       ...
                       _buildInfoRow(context, Icons.person_outline, 'Giới tính', tutor.displayGioiTinh),
                       // Optional: Sử dụng hàm format ngày
                       _buildInfoRow(context, Icons.cake_outlined, 'Ngày sinh', tutor.ngaySinh /* _formatDate(tutor.ngaySinh) */),
                     ],
                  ),
                ),
              ),
                ...
                child: Padding(
                  ...
                  child: Column(
                     ...
                     children: [
                       ...
                       const SizedBox(height: 15),
                       _buildInfoRow(context, Icons.school_outlined, 'Bằng cấp', tutor.bangCap),
                       _buildInfoRow(context, Icons.work_outline, 'Kinh nghiệm', tutor.kinhNghiem),
                       _buildInfoRow(context, Icons.book_outlined, 'Chuyên môn', tutor.subject),
                     ],
                  ),
                ),
              ),
  ...
  // Hàm xử lý gọi điện
   Future<void> _makePhoneCall(BuildContext context, String? phoneNumber) async {
    ...
    try {
        ...
        if (context.mounted) {
         scaffoldMessenger.showSnackBar(
           SnackBar(content: Text('Không thể thực hiện cuộc gọi: $e')), // Hiển thị lỗi cụ thể hơn
         );
       }
    }
   }
  ...
  // Hàm xử lý gửi email
   Future<void> _sendEmail(BuildContext context, String? email) async {
    ...
    try {
        ...
        if (context.mounted) {
         scaffoldMessenger.showSnackBar(
           SnackBar(content: Text('Không thể mở ứng dụng email: $e')), // Hiển thị lỗi cụ thể hơn
         );
       }
    }
   }
